...
declare variable $infobox:query := data:matchesAnyQuery(cts:query(search:parse($data:q)));

(: Use SPARQL to get a list of all candidate resources, based on the resource types we support :)
declare variable $infobox:candidate-resource-iris as xs:string* :=
  sem:sparql("
...
declare variable $infobox:name-properties :=
